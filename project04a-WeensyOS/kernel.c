...
void kernel_start(const char* command) {
    ...
    memory_foreach(kernel_pagetable, PROC_START_ADDR, map_to_nobody);  
    ...
    //memory_foreach(kernel_pagetable, MEMSIZE_PHYSICAL, map_to_nobody);  
    memory_foreach(kernel_pagetable, PROC_START_ADDR, map_to_kernel_space);
    map_to_nobody(kernel_pagetable, (uintptr_t)NULL);  
...
void process_setup(pid_t pid, const char* program_name) {
    ...
    //memory_foreach(process_pagetable, MEMSIZE_PHYSICAL, map_to_nobody);  
    ...
    for (auto seg = pgm.begin(); seg != pgm.end(); ++seg) {
        for (uintptr_t a = round_down(seg.va(), PAGESIZE);
            ...
            memory_map(process_pagetable, a, (uintptr_t) phys_addr, PTE_PWU);
    ...
    for (auto seg = pgm.begin(); seg != pgm.end(); ++seg) {
        void *phys_addr = (void*) memory_virtual_to_physical(process_pagetable, seg.va());
        memset(phys_addr, 0, seg.size());
        memcpy(phys_addr, seg.data(), seg.data_size());
    }
    ...
    memory_map(process_pagetable, stack_addr, (uintptr_t) phys_addr, PTE_PWU);
...
int syscall_page_alloc(uintptr_t addr) {
    ...
    memory_map(current->pagetable, addr, (uintptr_t) phys_addr, PTE_PWU);
    //map_to_user_space(current->pagetable, addr);
    memset((void*) memory_virtual_to_physical(current->pagetable, addr), 0, PAGESIZE);
    return 0;
}
